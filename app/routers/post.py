...
def get_posts(db: Session = Depends(get_db),
    ...
    # Bring back all post with the total number of votes for all post
    # This is done using a JOIN query in sqlalchemy
...
def create_post(post: schemas.PostCreate, db: Session = Depends(get_db),
    ...
    new_post = models.Post(owner_id=current_user.id, **post.dict())  # Unpacking the data being passed
    db.add(new_post)
    db.commit()
    db.refresh(new_post)
    return new_post
...
# The 'id' represents a post parameter.
# FastAPI will search for any post matching that id
